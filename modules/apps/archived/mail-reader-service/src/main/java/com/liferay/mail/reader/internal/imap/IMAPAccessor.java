...
public class IMAPAccessor {
		...
		try {
			...
			for (Message jxMessage : jxMessages) {
				...
				String to = InternetAddressUtil.toString(
					jxMessage._getRecipients(Message.RecipientType.TO));
				String cc = InternetAddressUtil.toString(
					jxMessage._getRecipients(Message.RecipientType.CC));
				String bcc = InternetAddressUtil.toString(
					jxMessage._getRecipients(Message.RecipientType.BCC));
	...
	private Message[] _getMessagesByUID(
		...
		return uidFolder._getMessagesByUID(remoteMessageIds);
	}
	...
	private long _getUID(Folder jxFolder, Message jxMessage)
		...
		return uidFolder._getUID(jxMessage);
	}
