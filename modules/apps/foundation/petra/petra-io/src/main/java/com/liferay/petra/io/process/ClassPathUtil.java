...
import com.liferay.petra.io.URLCodec;
import com.liferay.petra.io.process.ProcessConfig.Builder;
...
public class ClassPathUtil {
	...
	public static void initializeClassPaths(
		ClassLoader classLoader, Class<?> appServerClass,
		Class<?> portalGlobalClass, String servletContextRealPath) {

		StringBundler sb = new StringBundler(8);

		String appServerGlobalClassPath = _buildClassPath(
			classLoader, appServerClass.getName());
		...
		String portalGlobalClassPath = _buildClassPath(
			classLoader, portalGlobalClass.getName());
		...
		if (servletContextRealPath != null) {
			sb.append(File.pathSeparator);
			sb.append(servletContextRealPath);
	...
	private static String _buildClassPath(
		...
		String pathOfClass = className.replaceAll(
		...
		path = path.replaceAll(StringPool.BACK_SLASH, StringPool.SLASH);
