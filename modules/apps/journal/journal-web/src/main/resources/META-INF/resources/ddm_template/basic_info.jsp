...
<c:choose>
	<c:when test="<%= journalEditDDMTemplateDisplayContext.isShowSpecificLanguageType() %>">
		...
		<aui:input
			name="language"
			type="hidden"
			value="<%= journalEditDDMTemplateDisplayContext.getLanguage() %>"
		/>
	</c:when>
	<c:otherwise>
		<aui:select
		...
		>
			...
			%>
				...
				<aui:option
					label="<%= journalEditDDMTemplateDisplayContext.getTemplateLanguageTypeLabel(curLangType) %>"
					selected="<%= Objects.equals(journalEditDDMTemplateDisplayContext.getLanguage(), curLangType) %>"
					value="<%= curLangType %>"
				/>
...
<aui:input
	...
	name="structure"
	type="resource"
	value="<%= (ddmStructure != null) ? ddmStructure.getName(locale) : StringPool.BLANK %>"
	wrapperCssClass='<%= ((ddmTemplate == null) || (ddmTemplate.getClassPK() == 0)) ? "mb-2" : StringPool.BLANK %>'
/>
...
<c:if test="<%= ddmTemplate != null %>">
	...
	<aui:input
		name="webDavURL"
		type="resource"
		value="<%= ddmTemplate.getWebDavURL(themeDisplay, WebDAVUtil.getStorageToken(portlet)) %>"
	/>
</c:if>
...
<aui:input
	...
	name="cacheable"
	value="<%= journalEditDDMTemplateDisplayContext.isCacheable() %>"
/>
