	...
	<c:when test='<%= Objects.equals(id, "layout-prototype") %>'>
		...
		<c:if test="<%= layoutPrototype != null %>">
			<div class="layout-type">
				...
				<aui:input
					helpMessage="if-enabled-this-page-will-inherit-changes-made-to-the-page-template"
					id='<%= "addLayoutLayoutPrototypeLinkEnabled" + layoutPrototype.getUuid() %>'
					label="inherit-changes"
					name='<%= "layoutPrototypeLinkEnabled" + layoutPrototype.getUuid() %>'
					type="toggle-switch"
					value="<%= PropsValues.LAYOUT_PROTOTYPE_LINK_ENABLED_DEFAULT %>"
				/>
			</div>
		</c:if>
	</c:when>
