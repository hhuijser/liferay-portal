	...
	public List<Lock> findByUuid(
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				sb.append(LockModelImpl.ORDER_BY_JPQL);
			}
	...
	public Lock updateImpl(Lock lock) {
		...
		if (isNew && (lock.getCreateDate() == null)) {
			...
			if (serviceContext == null) {
				...
				lock.setCreateDate(serviceContext.getCreateDate(now));
			}
		}
	...
	private static class LockModelArgumentsResolver
		...
		private static Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = lockModelImpl.getColumnOriginalValue(
						columnName);
				}
				else {
