...
<aui:fieldset>
	<aui:input
		name="<%= Constants.CMD %>"
		type="hidden"
		value="<%= LDAPSettingsConstants.CMD_UPDATE_EXPORT %>"
	/>

	<aui:input
		disabled="<%= ldapImportConfiguration.importUserPasswordAutogenerated() && ldapImportEnabled %>"
		id="ldapExportEnabled"
		label="enable-export"
		name='<%= "ldap--" + LDAPConstants.EXPORT_ENABLED + "--" %>'
		type="checkbox"
		value="<%= ldapExportEnabled %>"
	/>

	<aui:input
		id="ldapExportGroupEnabled"
		label="enable-group-export"
		name='<%= "ldap--" + LDAPConstants.EXPORT_GROUP_ENABLED + "--" %>'
		type="checkbox"
		value="<%= ldapExportGroupEnabled %>"
	/>
</aui:fieldset>