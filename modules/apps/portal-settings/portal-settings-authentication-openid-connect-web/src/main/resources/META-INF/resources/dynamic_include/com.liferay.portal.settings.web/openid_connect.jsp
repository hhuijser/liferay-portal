...
<aui:input
	id='<%= PortalUtil.generateRandomKey(request, "portal_settings_authentication_openid_connect") %>'
	name="<%= ActionRequest.ACTION_NAME %>"
	type="hidden"
	value="/portal_settings/openid_connect"
/>

<aui:fieldset>
	<aui:input
		label="enabled"
		name='<%= PortalSettingsOpenIdConnectConstants.FORM_PARAMETER_NAMESPACE + "enabled" %>'
		type="checkbox"
		value="<%= openIdConnectConfiguration.enabled() %>"
	/>
</aui:fieldset>