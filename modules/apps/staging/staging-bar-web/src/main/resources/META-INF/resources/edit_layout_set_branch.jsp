...
<div class="container-fluid-1280 container-view" data-namespace="<portlet:namespace />" id="<portlet:namespace /><%= (layoutSetBranch != null) ? "updateBranch" : "addBranch" %>">
	<aui:model-context
		bean="<%= layoutSetBranch %>"
		model="<%= LayoutSetBranch.class %>"
	/>
	...
	>
		...
		<aui:fieldset>
			...
			<c:if test="<%= layoutSetBranch == null %>">
				...
				>
					...
					<aui:option
						label="none-empty-site-pages-variation"
						value="<%= LayoutSetBranchConstants.NO_BRANCHES %>"
					/>
		...
		<aui:button-row>
			<aui:button
				type="submit"
				value='<%= (layoutSetBranch != null) ? "update" : "add" %>'
			/>
