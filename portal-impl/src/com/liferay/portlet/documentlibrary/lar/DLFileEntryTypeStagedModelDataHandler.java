...
public class DLFileEntryTypeStagedModelDataHandler
	...
	protected void doImportStagedModel(
		...
		if (portletDataContext.isDataStrategyMirror()) {
			...
			ddmStructureIdsArray = importFileEntryTypeStructures(
				portletDataContext, fileEntryType, existingDLFileEntryType,
				preloaded);
			...
			ddmStructureIdsArray = importFileEntryTypeStructures(
				portletDataContext, fileEntryType, null, preloaded);
	...
	protected long[] importFileEntryTypeStructures(
			PortletDataContext portletDataContext,
			DLFileEntryType fileEntryType,
			DLFileEntryType existingfileEntryType, boolean preloaded)
		throws PortletDataException, SystemException {

		preparePreloadedStructure(
			fileEntryType, existingfileEntryType, preloaded);
		...
		return ddmStructureIdsArray;
	}

	protected void preparePreloadedStructure(
			DLFileEntryType fileEntryType,
			DLFileEntryType existingFileEntryType, boolean preloaded)
		throws PortletDataException, SystemException {

		if (!preloaded || (existingFileEntryType == null)) {
			return;
		}

		String ddmStructureKey = DLUtil.getDDMStructureKey(fileEntryType);

		List<DDMStructure> existingStructures =
			DDMStructureLocalServiceUtil.getDLFileEntryTypeStructures(
				existingFileEntryType.getFileEntryTypeId());

		for (DDMStructure existingStructure : existingStructures) {
			if (!DLUtil.isAutoGeneratedDLFileEntryTypeDDMStructureKey(
			...
			existingStructure.setStructureKey(ddmStructureKey);

			DDMStructureLocalServiceUtil.updateDDMStructure(existingStructure);
