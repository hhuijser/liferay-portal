...
<%
boolean editorHasUpdateFieldPermission = UsersAdminUtil.hasUpdateFieldPermission(themeDisplay.getUser(), StringPool.BLANK, true);
...
<div class="row-fluid">
	<aui:fieldset cssClass="span6">
		...
		<c:if test="<%= !PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || (selUser != null) %>">
			<c:choose>
				<c:when test='<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || !(editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "screenName", false)) %>'>
		...
		<c:choose>
			<c:when test='<%= !(editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "emailAddress", false)) %>'>
	...
	<aui:fieldset cssClass="span5">
		<div>
			<c:if test="<%= selUser != null %>">
				<c:choose>
					<c:when test='<%= editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "portrait", false) %>'>
		...
		<c:choose>
			<c:when test="<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.FIELD_ENABLE_COM_LIFERAY_PORTAL_MODEL_CONTACT_BIRTHDAY) %>">
				...
				<aui:input bean="<%= selContact %>" cssClass="modify-link" disabled='<%= !(editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "birthday", false)) %>' model="<%= Contact.class %>" name="birthday" value="<%= birthday %>" />
			</c:when>
			<c:otherwise>
		...
		<c:if test="<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.FIELD_ENABLE_COM_LIFERAY_PORTAL_MODEL_CONTACT_MALE) %>">
			<aui:select bean="<%= selContact %>" disabled='<%= !(editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "gender", false)) %>' label="gender" model="<%= Contact.class %>" name="male">
				<aui:option label="male" value="true" />
				<aui:option label="female" value="false" />
			</aui:select>
		</c:if>

		<aui:input disabled='<%= !(editorHasUpdateFieldPermission || UsersAdminUtil.hasUpdateFieldPermission(selUser, "jobTitle", false)) %>' name="jobTitle" />
	</aui:fieldset>
</div>